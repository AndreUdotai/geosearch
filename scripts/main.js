...
apiRequest.onreadystatechange = () => {
	if(apiRequest.readyState === 4) {
		...
		if(postCode != undefined){
			...
			postalCode.textContent	= 'Postal Code is not available for this city.'
		}
		longitude.innerText			= 'Longitude: ' + longitudeCoordinate;
		latitude.innerText			= 'Latitude: ' + latitudeCoordinate + ' | ';


		// (postCode != undefined) ? 'Postal code: ' + poseCode : 'Postal code is not available for this city';
...
function landmarkRetrieve(lat, long){
	...
	landmarkRequest.onreadystatechange = () => {
		if(landmarkRequest.readyState === 4) {
			...
			let landmarkCard1 = document.getElementById('landmarkCard1');
			let landmarkCard2 = document.getElementById('landmarkCard2');
			let landmarkCard3 = document.getElementById('landmarkCard3');
			let landmarkCard4 = document.getElementById('landmarkCard4');


			checkUndefined(response.Response.View[0].Result[0], landmark1Name, landmarkCard1);
			checkUndefined(response.Response.View[0].Result[1], landmark2Name, landmarkCard2);
			checkUndefined(response.Response.View[0].Result[2], landmark3Name, landmarkCard3);
			checkUndefined(response.Response.View[0].Result[3], landmark4Name, landmarkCard4);
...
let checkUndefined = (l,ln, lc) => {
	if(l != undefined)
		lc.style.display	= 'inline';
		ln.innerText		= l.Location.Name;
}


// // Using Tenary operator
// let checkUndefined = (l, ln, ld) => {
// 	l != undefined ? lc.style.display = 'inline' : 
// }